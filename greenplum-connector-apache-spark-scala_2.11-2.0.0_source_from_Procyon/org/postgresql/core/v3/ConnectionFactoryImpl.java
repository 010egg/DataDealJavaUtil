...
public class ConnectionFactoryImpl extends ConnectionFactory
{
    ...
    private PGStream tryConnect(final String user, final String database, final Properties info, final SocketFactory socketFactory, final HostSpec hostSpec, final SslMode sslMode) throws SQLException, IOException {
    ...
    public QueryExecutor openConnectionImpl(final HostSpec[] hostSpecs, final String user, final String database, final Properties info) throws SQLException {
        ...
        final String targetServerTypeStr = PGProperty.TARGET_SERVER_TYPE.get(info);
                ...
                try {
                    try {
                        ...
                        if (sslMode == SslMode.PREFER && PSQLState.INVALID_AUTHORIZATION_SPECIFICATION.getState().equals(e.getSQLState())) {
                            ...
                            if (ex != null) {
                                log(Level.FINE, "sslMode==PREFER, however non-SSL connection failed as well", ex, new Object[0]);
                                e.addSuppressed(ex);
                                throw e;
                            }
                        }
                            ...
                            if (ex != null) {
                                log(Level.FINE, "sslMode==ALLOW, however SSL connection failed as well", ex, new Object[0]);
                                e.addSuppressed(ex);
                                throw e;
                            }
                        }
                    ...
                    case 82: {
                        ...
                        switch (areq) {
                            ...
                            case 9: {
                                ...
                                MakeGSS.authenticate(pgStream, host, user, password, PGProperty.JAAS_APPLICATION_NAME.get(info), PGProperty.KERBEROS_SERVER_NAME.get(info), usespnego, PGProperty.JAAS_LOGIN.getBoolean(info), PGProperty.LOG_SERVER_ERROR_DETAIL.getBoolean(info));
                                continue;
                            }
                            case 8: {
    ...
    private boolean isPrimary(final QueryExecutor queryExecutor) throws SQLException, IOException {
        ...
        final String value = queryExecutor.getEncoding().decode(results.get(0));
