...
public class PgResultSet implements ResultSet, PGRefCursorResultSet
{
            ...
            default: {
                ...
                if (type.equals("uuid")) {
                    if (this.isBinary(columnIndex)) {
                        return this.getUUID(this.thisRow.get(columnIndex - 1));
                    }
                    ...
                    if (this.isBinary(columnIndex)) {
                        return HStoreConverter.fromBytes(this.thisRow.get(columnIndex - 1), this.connection.getEncoding());
                    }
    ...
    public Array getArray(final int i) throws SQLException {
        ...
        if (this.isBinary(i)) {
            return this.makeArray(oid, this.thisRow.get(i - 1));
        }
    ...
    public Date getDate(final int i, Calendar cal) throws SQLException {
        ...
        if (oid == 1082) {
            return this.connection.getTimestampUtils().toDateBin(tz, this.thisRow.get(col));
        }
    ...
    public Time getTime(final int i, Calendar cal) throws SQLException {
        ...
        if (oid == 1083 || oid == 1266) {
            return this.connection.getTimestampUtils().toTimeBin(tz, this.thisRow.get(col));
        }
        if (oid != 1114 && oid != 1184) {
    ...
    private LocalTime getLocalTime(final int i) throws SQLException {
        ...
        final int oid = this.fields[col].getOID();
        if (oid == 1083) {
            return this.connection.getTimestampUtils().toLocalTimeBin(this.thisRow.get(col));
        }
        throw new PSQLException(GT.tr("Cannot convert the column of type {0} to requested type {1}.", Oid.toString(oid), "time"), PSQLState.DATA_TYPE_MISMATCH);
    }
    ...
    public Timestamp getTimestamp(final int i, Calendar cal) throws SQLException {
        ...
        if (this.isBinary(i)) {
            if (oid == 1184 || oid == 1114) {
                ...
                final TimeZone tz = cal.getTimeZone();
                return this.connection.getTimestampUtils().toTimestampBin(tz, this.thisRow.get(col), hasTimeZone);
            }
            long millis;
            if (oid == 1083 || oid == 1266) {
    ...
    private OffsetDateTime getOffsetDateTime(final int i) throws SQLException {
        ...
        if (this.isBinary(i)) {
            if (oid == 1184 || oid == 1114) {
                return this.connection.getTimestampUtils().toOffsetDateTimeBin(this.thisRow.get(col));
            }
            if (oid == 1266) {
    ...
    private LocalDateTime getLocalDateTime(final int i) throws SQLException {
        ...
        if (this.isBinary(i)) {
            return this.connection.getTimestampUtils().toLocalDateTimeBin(this.thisRow.get(col));
        }
        final String string = this.getString(i);
    ...
    public boolean isBeforeFirst() throws SQLException {
        this.checkClosed();
        return !this.onInsertRow && this.rowOffset + this.currentRow < 0 && !this.rows.isEmpty();
    }
    ...
    public synchronized void insertRow() throws SQLException {
        ...
        final Iterator<String> columnNames = this.updateValues.keySet().iterator();
        ...
        this.insertStatement = this.connection.prepareStatement(insertSQL.toString(), 1);
        final Iterator<Object> values = this.updateValues.values().iterator();
        int j = 1;
        while (values.hasNext()) {
            this.insertStatement.setObject(j, values.next());
            ++j;
        }
        this.insertStatement.executeUpdate();
        if (this.usingOID) {
            final long insertedOID = ((PgStatement)this.insertStatement).getLastOID();
            this.updateValues.put("oid", insertedOID);
        }
        this.updateRowBuffer(true);
    ...
    public void refreshRow() throws SQLException {
        ...
        final PgResultSet rs = (PgResultSet)this.selectStatement.executeQuery();
        if (rs.next()) {
            this.rowBuffer = rs.thisRow;
        }
        this.rows.set(this.currentRow, this.rowBuffer);
        this.thisRow = this.rowBuffer;
    ...
    public synchronized void updateRow() throws SQLException {
        ...
        final int numColumns = this.updateValues.size();
        final Iterator<String> columns = this.updateValues.keySet().iterator();
        int i = 0;
        ...
        for (final Object o : this.updateValues.values()) {
            this.updateStatement.setObject(k + 1, o);
            ++k;
        }
        for (int l = 0; l < numKeys; ++l, ++k) {
            this.updateStatement.setObject(k + 1, this.primaryKeys.get(l).getValue());
        }
        this.updateStatement.executeUpdate();
        this.updateStatement.close();
        this.updateStatement = null;
        this.updateRowBuffer(false);
        this.connection.getLogger().log(Level.FINE, "copying data");
        this.thisRow = this.rowBuffer.readOnlyCopy();
        this.rows.set(this.currentRow, this.rowBuffer);
        this.connection.getLogger().log(Level.FINE, "done updates");
        this.updateValues.clear();
        this.doingUpdates = false;
    ...
    private void parseQuery() {
        final String sql = this.originalQuery.toString(null);
    ...
    private void setRowBufferColumn(final int columnIndex, final Object valueObject) throws SQLException {
        if (valueObject instanceof PGobject) {
            final String value = ((PGobject)valueObject).getValue();
            this.rowBuffer.set(columnIndex, (byte[])((value == null) ? null : this.connection.encodeString(value)));
        }
        else {
            switch (this.getSQLType(columnIndex + 1)) {
                case -7:
                case 16: {
                    this.rowBuffer.set(columnIndex, this.connection.encodeString(((boolean)valueObject) ? "t" : "f"));
                    return;
                }
                case 91: {
                    this.rowBuffer.set(columnIndex, this.connection.encodeString(this.connection.getTimestampUtils().toString(this.getDefaultCalendar(), (Date)valueObject)));
                    return;
                }
                case 92: {
                    this.rowBuffer.set(columnIndex, this.connection.encodeString(this.connection.getTimestampUtils().toString(this.getDefaultCalendar(), (Time)valueObject)));
                    return;
                }
                case 93: {
                    this.rowBuffer.set(columnIndex, this.connection.encodeString(this.connection.getTimestampUtils().toString(this.getDefaultCalendar(), (Timestamp)valueObject)));
                    return;
                }
                case 0: {
                    return;
                ...
                case -2: {
                    if (this.isBinary(columnIndex + 1)) {
                        this.rowBuffer.set(columnIndex, (byte[])valueObject);
                        return;
                    }
                    try {
                        this.rowBuffer.set(columnIndex, PGbytea.toPGString((byte[])valueObject).getBytes("ISO-8859-1"));
                        return;
                    }
    ...
    private void updateRowBuffer(final boolean isInsert) throws SQLException {
        for (final Map.Entry<String, Object> entry : this.updateValues.entrySet()) {
        ...
        if (isInsert) {
            ...
            try {
                generatedKeys.next();
                for (int numKeys = this.primaryKeys.size(), i = 0; i < numKeys; ++i) {
                    final PrimaryKey key = this.primaryKeys.get(i);
                    final int columnIndex2 = key.index - 1;
                    final Object valueObject2 = generatedKeys.getObject(key.name);
                    this.setRowBufferColumn(columnIndex2, valueObject2);
                }
            }
            finally {
                generatedKeys.close();
    ...
    public boolean next() throws SQLException {
        ...
        if (this.currentRow + 1 >= this.rows.size()) {
            if (this.cursor == null || (this.maxRows > 0 && this.rowOffset + this.rows.size() >= this.maxRows)) {
    ...
    public String getString(final int columnIndex) throws SQLException {
        ...
        if (this.isBinary(columnIndex) && this.getSQLType(columnIndex) != 12) {
            ...
            try {
                return this.trimString(columnIndex, encoding.decode(this.thisRow.get(columnIndex - 1)));
            }
    ...
    public boolean getBoolean(final int columnIndex) throws SQLException {
        ...
        if (16 == this.fields[col].getOID()) {
            final byte[] v = this.thisRow.get(col);
            return 1 == v.length && 116 == v[0];
        }
        if (this.isBinary(columnIndex)) {
            return BooleanTypeUtil.castToBoolean(this.readDoubleValue(this.thisRow.get(col), this.fields[col].getOID(), "boolean"));
        }
        return BooleanTypeUtil.castToBoolean(this.getString(columnIndex));
    }
    ...
    public byte getByte(final int columnIndex) throws SQLException {
        ...
        if (this.isBinary(columnIndex)) {
            final int col = columnIndex - 1;
            return (byte)this.readLongValue(this.thisRow.get(col), this.fields[col].getOID(), -128L, 127L, "byte");
        }
    ...
    public short getShort(final int columnIndex) throws SQLException {
        ...
        final int oid = this.fields[col].getOID();
        if (oid == 21) {
            return ByteConverter.int2(this.thisRow.get(col), 0);
        }
        return (short)this.readLongValue(this.thisRow.get(col), oid, -32768L, 32767L, "short");
    }
    ...
    public int getInt(final int columnIndex) throws SQLException {
        ...
        final int oid = this.fields[col].getOID();
        if (oid == 23) {
            return ByteConverter.int4(this.thisRow.get(col), 0);
        }
        return (int)this.readLongValue(this.thisRow.get(col), oid, -2147483648L, 2147483647L, "int");
    }
    ...
    public long getLong(final int columnIndex) throws SQLException {
        ...
        final int oid = this.fields[col].getOID();
        if (oid == 20) {
            return ByteConverter.int8(this.thisRow.get(col), 0);
        }
        return this.readLongValue(this.thisRow.get(col), oid, Long.MIN_VALUE, Long.MAX_VALUE, "long");
    }
    
    private long getFastLong(final int columnIndex) throws SQLException, NumberFormatException {
        final byte[] bytes = this.thisRow.get(columnIndex - 1);
        if (bytes.length == 0) {
    ...
    public float getFloat(final int columnIndex) throws SQLException {
        ...
        final int oid = this.fields[col].getOID();
        if (oid == 700) {
            return ByteConverter.float4(this.thisRow.get(col), 0);
        }
        return (float)this.readDoubleValue(this.thisRow.get(col), oid, "float");
    }
    ...
    public double getDouble(final int columnIndex) throws SQLException {
        ...
        final int oid = this.fields[col].getOID();
        if (oid == 701) {
            return ByteConverter.float8(this.thisRow.get(col), 0);
        }
        return this.readDoubleValue(this.thisRow.get(col), oid, "double");
    }
    ...
    private Number getNumeric(final int columnIndex, final int scale, final boolean allowNaN) throws SQLException {
        ...
        if (this.isBinary(columnIndex)) {
            ...
            if (sqlType != 2 && sqlType != 3) {
                ...
                final Number num = ByteConverter.numeric(this.thisRow.get(columnIndex - 1));
            ...
            if (encoding.hasAsciiNumbers()) {
                try {
                    BigDecimal res2 = this.getFastBigDecimal(columnIndex);
    ...
    public byte[] getBytes(final int columnIndex) throws SQLException {
        ...
        if (this.fields[columnIndex - 1].getOID() == 17) {
            return this.trimBytes(columnIndex, PGbytea.toBytes(this.thisRow.get(columnIndex - 1)));
        }
        return this.trimBytes(columnIndex, this.thisRow.get(columnIndex - 1));
    }
    ...
    public Object getObject(final int columnIndex) throws SQLException {
        ...
        if (this.isBinary(columnIndex)) {
            return this.connection.getObject(this.getPGType(columnIndex), null, this.thisRow.get(columnIndex - 1));
        }
    ...
    private void initSqlType(final Field field) throws SQLException {
        ...
        final int oid = field.getOID();
        final String pgType = typeInfo.getPGType(oid);
        ...
        if (type == BigDecimal.class) {
            ...
            if (type == LocalDate.class) {
                ...
                if (PGobject.class.isAssignableFrom(type)) {
                    Object object;
                    if (this.isBinary(columnIndex)) {
                        object = this.connection.getObject(this.getPGType(columnIndex), null, this.thisRow.get(columnIndex - 1));
                    }
                    else {
