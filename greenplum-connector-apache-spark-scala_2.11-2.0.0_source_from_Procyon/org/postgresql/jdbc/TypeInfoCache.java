...
public class TypeInfoCache implements TypeInfo
{
    ...
    public void cacheSQLTypes() throws SQLException {
        ...
        if (!((BaseStatement)this.getAllTypeInfoStatement).executeWithFlags(16)) {
            throw new PSQLException(GT.tr("No results were returned by the query.", new Object[0]), PSQLState.NO_DATA);
        }
        final ResultSet rs = this.getAllTypeInfoStatement.getResultSet();
        while (rs.next()) {
            final String typeName = rs.getString("typname");
    ...
    public synchronized int getSQLType(final String pgTypeName) throws SQLException {
        ...
        if (!((BaseStatement)this.getTypeInfoStatement).executeWithFlags(16)) {
            throw new PSQLException(GT.tr("No results were returned by the query.", new Object[0]), PSQLState.NO_DATA);
        }
        final ResultSet rs = this.getTypeInfoStatement.getResultSet();
        Integer type = 1111;
        if (rs.next()) {
    ...
    public synchronized int getPGType(final String pgTypeName) throws SQLException {
        ...
        oid = 0;
        final ResultSet rs = oidStatement.getResultSet();
        if (rs.next()) {
            oid = (int)rs.getLong(1);
            final String internalName = rs.getString(2);
    ...
    public synchronized String getPGType(final int oid) throws SQLException {
        ...
        if (!((BaseStatement)this.getNameStatement).executeWithFlags(16)) {
            throw new PSQLException(GT.tr("No results were returned by the query.", new Object[0]), PSQLState.NO_DATA);
        }
        final ResultSet rs = this.getNameStatement.getResultSet();
        if (rs.next()) {
    ...
    public synchronized char getArrayDelimiter(final int oid) throws SQLException {
        ...
        if (!((BaseStatement)this.getArrayDelimiterStatement).executeWithFlags(16)) {
            throw new PSQLException(GT.tr("No results were returned by the query.", new Object[0]), PSQLState.NO_DATA);
        }
        final ResultSet rs = this.getArrayDelimiterStatement.getResultSet();
        if (!rs.next()) {
            throw new PSQLException(GT.tr("No results were returned by the query.", new Object[0]), PSQLState.NO_DATA);
        }
    ...
    public synchronized int getPGArrayElement(final int oid) throws SQLException {
        ...
        if (!((BaseStatement)this.getArrayElementOidStatement).executeWithFlags(16)) {
            throw new PSQLException(GT.tr("No results were returned by the query.", new Object[0]), PSQLState.NO_DATA);
        }
        final ResultSet rs = this.getArrayElementOidStatement.getResultSet();
        if (!rs.next()) {
            throw new PSQLException(GT.tr("No results were returned by the query.", new Object[0]), PSQLState.NO_DATA);
        }
