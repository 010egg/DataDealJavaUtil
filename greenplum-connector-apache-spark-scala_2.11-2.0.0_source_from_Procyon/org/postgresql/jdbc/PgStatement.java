...
public class PgStatement implements Statement, BaseStatement
{
    ...
    protected void closeForNextExecution() throws SQLException {
        ...
        synchronized (this) {
            while (this.firstUnclosedResult != null) {
                final PgResultSet rs = (PgResultSet)this.firstUnclosedResult.getResultSet();
                if (rs != null) {
                    rs.closeInternally();
                }
    ...
    public void addBatch(final String sql) throws SQLException {
        ...
        if (this.batchStatements == null) {
            this.batchStatements = new ArrayList<Query>();
            this.batchParameters = new ArrayList<ParameterList>();
        }
        final boolean shouldUseParameterized = false;
    ...
    private BatchResultHandler internalExecuteBatch() throws SQLException {
        ...
        final Query[] queries = this.batchStatements.toArray(new Query[0]);
        final ParameterList[] parameterLists = this.batchParameters.toArray(new ParameterList[0]);
        this.batchStatements.clear();
        this.batchParameters.clear();
