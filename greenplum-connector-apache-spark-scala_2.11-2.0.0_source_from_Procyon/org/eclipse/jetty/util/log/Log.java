...
public class Log
{
    ...
    private static void loadProperties(final String resourceName, final Properties props) {
    ...
    public static void initialized() {
        synchronized (Log.class) {
            ...
            try {
                final Class<?> log_class = (Class<?>)Loader.loadClass(Log.class, Log.__logClass);
                if (Log.LOG == null || !Log.LOG.getClass().equals(log_class)) {
                    (Log.LOG = (Logger)log_class.newInstance()).debug("Logging to {} via {}", Log.LOG, log_class.getName());
                }
            }
            catch (Throwable e) {
    ...
    private static void initStandardLogging(final Throwable e) {
        ...
        if (Log.LOG == null) {
            ...
            (Log.LOG = new StdErrLog()).debug("Logging to {} via {}", Log.LOG, log_class.getName());
        }
    }
    ...
    static {
        ...
        AccessController.doPrivileged((PrivilegedAction<Object>)new PrivilegedAction<Object>() {
            @Override
            public Object run() {
                loadProperties("jetty-logging.properties", Log.__props);
                String osName = System.getProperty("os.name");
                if (osName != null && osName.length() > 0) {
                    osName = osName.toLowerCase(Locale.ENGLISH).replace(' ', '-');
                    loadProperties("jetty-logging-" + osName + ".properties", Log.__props);
                }
