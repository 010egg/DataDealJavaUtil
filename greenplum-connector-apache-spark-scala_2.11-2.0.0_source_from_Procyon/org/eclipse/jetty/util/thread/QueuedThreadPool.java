...
public class QueuedThreadPool extends AbstractLifeCycle implements ThreadPool.SizedThreadPool, Dumpable
{
    ...
    public QueuedThreadPool(@Name("maxThreads") final int maxThreads, @Name("minThreads") final int minThreads, @Name("idleTimeout") final int idleTimeout, @Name("queue") BlockingQueue<Runnable> queue) {
    ...
    public void dump(final Appendable out, final String indent) throws IOException {
        final List<Object> dump = new ArrayList<Object>(this.getMaxThreads());
        for (final Thread thread : this._threads) {
            ...
            if (this.isDetailedDump()) {
                ...
                dump.add(thread.getId() + " " + thread.getName() + " " + thread.getState() + " @ " + ((trace.length > 0) ? trace[0] : "???") + (idle ? " IDLE" : "") + ((p == 5) ? "" : (" prio=" + p)));
            }
        }
