...
public class HttpParser
{
    ...
    private boolean handleKnownHeaders(final ByteBuffer buffer) {
        ...
        switch (this._header) {
            ...
            case TRANSFER_ENCODING: {
                ...
                if (this._valueString.contains(HttpHeaderValue.CHUNKED.toString())) {
            ...
            case HOST: {
                ...
                if (host == null || host.length() == 0) {
                    throw new BadMessageException(400, "Bad Host header");
                }
                int i;
                int len = i = host.length();
            Label_0407:
                while (i-- > 0) {
                    final char c2 = (char)('\u00ff' & host.charAt(i));
                    switch (c2) {
                        case ']': {
                            break Label_0407;
                        }
                ...
                if (host.charAt(0) == '[') {
                    if (host.charAt(len - 1) != ']') {
                        throw new BadMessageException(400, "Bad IPv6 Host header");
                    }
    ...
    static {
        ...
        (CACHE = new ArrayTrie<HttpField>(2048)).put(new HttpField(HttpHeader.CONNECTION, HttpHeaderValue.CLOSE));
        ...
        for (final String type : new String[] { "text/plain", "text/html", "text/xml", "text/json", "application/json", "application/x-www-form-urlencoded" }) {
            ...
            for (final String charset : new String[] { "UTF-8", "ISO-8859-1" }) {
                HttpParser.CACHE.put(new HttpGenerator.CachedHttpField(HttpHeader.CONTENT_TYPE, type + ";charset=" + charset));
                HttpParser.CACHE.put(new HttpGenerator.CachedHttpField(HttpHeader.CONTENT_TYPE, type + "; charset=" + charset));
            }
        }
