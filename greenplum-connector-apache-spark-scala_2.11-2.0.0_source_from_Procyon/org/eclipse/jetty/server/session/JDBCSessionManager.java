...
public class JDBCSessionManager extends AbstractSessionManager
{
    ...
    protected Session loadSession(final String id, final String canonicalContextPath, final String vhost) throws Exception {
        ...
        final Runnable load = new Runnable() {
            @Override
            public void run() {
                try (final Connection connection = JDBCSessionManager.this.getConnection();
                    ...
                    if (result.next()) {
                        ...
                        session.setCookieSet(result.getLong(JDBCSessionManager.this._sessionTableSchema.getCookieTimeColumn()));
                        ...
                        try (final InputStream is = ((JDBCSessionIdManager)JDBCSessionManager.this.getSessionIdManager())._dbAdaptor.getBlobInputStream(result, JDBCSessionManager.this._sessionTableSchema.getMapColumn());
    ...
    public class Session extends MemSession
    {
        ...
        public void setCookieSet(final long ms) {
            this._cookieSet = ms;
        }
        ...
        public String toString() {
            return "Session rowId=" + this._rowId + ",id=" + this.getId() + ",lastNode=" + this._lastNode + ",created=" + this.getCreationTime() + ",accessed=" + this.getAccessed() + ",lastAccessed=" + this.getLastAccessedTime() + ",cookieSet=" + this._cookieSet + ",maxInterval=" + this.getMaxInactiveInterval() + ",lastSaved=" + this._lastSaved + ",expiry=" + this._expiryTime;
        }
    }
}
