...
public class ServerConnector extends AbstractNetworkConnector
{
    ...
    public ServerConnector(@Name("server") final Server server, @Name("executor") final Executor executor, @Name("scheduler") final Scheduler scheduler, @Name("bufferPool") final ByteBufferPool bufferPool, @Name("acceptors") final int acceptors, @Name("selectors") final int selectors, @Name("factories") final ConnectionFactory... factories) {
        ...
        this._lingerTime = -1;
        this.addBean(this._manager = new ServerConnectorManager(this.getExecutor(), this.getScheduler(), (selectors > 0) ? selectors : Math.max(1, Math.min(4, Runtime.getRuntime().availableProcessors() / 2))), true);
    }
    ...
    protected SelectChannelEndPoint newEndPoint(final SocketChannel channel, final SelectorManager.ManagedSelector selectSet, final SelectionKey key) throws IOException {
    ...
    {
        private ServerConnectorManager(final Executor executor, final Scheduler scheduler, final int selectors) {
