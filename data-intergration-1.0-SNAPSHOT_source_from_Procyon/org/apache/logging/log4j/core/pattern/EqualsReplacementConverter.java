...
public final class EqualsReplacementConverter extends LogEventPatternConverter
{
    ...
    public void format(final LogEvent event, final StringBuilder toAppendTo) {
        ...
        toAppendTo.append(this.testString.equals(string) ? this.parseSubstitution(event) : string);
    }
    
    String parseSubstitution(final LogEvent event) {
        final StringBuilder substitutionBuffer = new StringBuilder();
        if (this.substitution.contains("%")) {
            final List<PatternFormatter> substitutionFormatters = this.parser.parse(this.substitution);
