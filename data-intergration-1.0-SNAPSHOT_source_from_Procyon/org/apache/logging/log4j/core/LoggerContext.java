...
public class LoggerContext extends AbstractLifeCycle implements org.apache.logging.log4j.spi.LoggerContext, AutoCloseable, Terminable, ConfigurationListener
{
    ...
    public boolean stop(final long timeout, final TimeUnit timeUnit) {
        ...
        try {
            ...
            final String source = "LoggerContext '" + this.getName() + "'";
            shutdownEs = ExecutorServices.shutdown(this.executorService, timeout, timeUnit, source);
            shutdownEsd = ExecutorServices.shutdown(this.executorServiceDeamons, -1L, timeUnit, source);
        }
        finally {
            this.configLock.unlock();
            this.setStopped();
        }
        LoggerContext.LOGGER.debug("Stopped LoggerContext[name={}, {}]...", this.getName(), this);
    ...
    private Configuration setConfiguration(final Configuration config) {
        ...
        try {
            ...
            this.executorService = Executors.newCachedThreadPool(Log4jThreadFactory.createThreadFactory(this.contextName));
            this.executorServiceDeamons = Executors.newCachedThreadPool(Log4jThreadFactory.createDaemonThreadFactory(this.contextName));
    ...
    public ExecutorService getExecutorServiceDeamons() {
        return this.executorServiceDeamons;
    }
    ...
    public Future<?> submitDaemon(final Runnable task) {
        return this.executorServiceDeamons.submit(task);
    }
