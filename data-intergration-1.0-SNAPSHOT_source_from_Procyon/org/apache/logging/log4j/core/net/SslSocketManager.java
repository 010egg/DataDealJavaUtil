...
public class SslSocketManager extends TcpSocketManager
{
    ...
    public static SslSocketManager getSocketManager(final SslConfiguration sslConfig, final String host, int port, final int connectTimeoutMillis, int reconnectDelayMillis, final boolean immediateFail, final Layout<? extends Serializable> layout, final int bufferSize) {
        ...
        return (SslSocketManager)OutputStreamManager.getManager("TLS:" + host + ':' + port, new SslFactoryData(sslConfig, host, port, connectTimeoutMillis, reconnectDelayMillis, immediateFail, layout, bufferSize), SslSocketManager.FACTORY);
    }
    ...
    private static class SslFactoryData
    {
        ...
        public SslFactoryData(final SslConfiguration sslConfiguration, final String host, final int port, final int connectTimeoutMillis, final int delayMillis, final boolean immediateFail, final Layout<? extends Serializable> layout, final int bufferSize) {
            this.host = host;
            this.port = port;
    ...
    private static class SslSocketManagerFactory implements ManagerFactory<SslSocketManager, SslFactoryData>
    {
        ...
        public SslSocketManager createManager(final String name, final SslFactoryData data) {
            ...
            return new SslSocketManager(name, os, socket, data.sslConfiguration, inetAddress, data.host, data.port, 0, data.delayMillis, data.immediateFail, data.layout, data.bufferSize);
        }
