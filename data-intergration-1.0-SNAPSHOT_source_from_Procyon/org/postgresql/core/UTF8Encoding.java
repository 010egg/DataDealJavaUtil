...
class UTF8Encoding extends Encoding
{
    ...
    UTF8Encoding(final String jvmEncoding) {
        super(jvmEncoding);
    ...
    private static final void checkByte(final int ch, final int pos, final int len) throws IOException {
        if ((ch & 0xC0) != 0x80) {
            throw new IOException(GT.tr("Illegal UTF-8 sequence: byte {0} of {1} byte sequence is not 10xxxxxx: {2}", new Object[] { new Integer(pos), new Integer(len), new Integer(ch) }));
        }
    }
    ...
    private static final void checkMinimal(final int ch, final int minValue) throws IOException {
        ...
        throw new IOException(GT.tr("Illegal UTF-8 sequence: {0} bytes used to encode a {1} byte value: {2}", new Object[] { new Integer(actualLen), new Integer(expectedLen), new Integer(ch) }));
    }
    ...
    public synchronized String decode(final byte[] data, final int offset, final int length) throws IOException {
        ...
        try {
            while (in < end) {
                ...
                if (ch >= 128) {
                    if (ch < 192) {
                        throw new IOException(GT.tr("Illegal UTF-8 sequence: initial byte is {0}: {1}", new Object[] { "10xxxxxx", new Integer(ch) }));
                    }
                    if (ch < 224) {
                        ch = (ch & 0x1F) << 6;
                        ...
                        if (ch >= 248) {
                            throw new IOException(GT.tr("Illegal UTF-8 sequence: initial byte is {0}: {1}", new Object[] { "11111xxx", new Integer(ch) }));
                        }
                        ch = (ch & 0x7) << 18;
                ...
                if (ch > 1114111) {
                    throw new IOException(GT.tr("Illegal UTF-8 sequence: final value is out of range: {0}", new Integer(ch)));
                }
                if (ch > 65535) {
                    ch -= 65536;
                    ...
                    if (ch >= 55296 && ch < 57344) {
                        throw new IOException(GT.tr("Illegal UTF-8 sequence: final value is a surrogate value: {0}", new Integer(ch)));
                    }
                    cdata[out++] = (char)ch;
                }
            }
