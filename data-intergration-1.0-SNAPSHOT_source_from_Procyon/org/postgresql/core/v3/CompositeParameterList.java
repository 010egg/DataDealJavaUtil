...
class CompositeParameterList implements V3ParameterList
{
    ...
    private final int findSubParam(final int index) throws SQLException {
        if (index < 1 || index > this.total) {
            throw new PSQLException(GT.tr("The column index is out of range: {0}, number of columns: {1}.", new Object[] { new Integer(index), new Integer(this.total) }), PSQLState.INVALID_PARAMETER_VALUE);
        }
    ...
    public void checkAllParametersSet() throws SQLException {
        for (int sub = 0; sub < this.subparams.length; ++sub) {
            this.subparams[sub].checkAllParametersSet();
        }
    }
    ...
    public void convertFunctionOutParameters() {
        for (int sub = 0; sub < this.subparams.length; ++sub) {
            this.subparams[sub].convertFunctionOutParameters();
        }
    }
}
