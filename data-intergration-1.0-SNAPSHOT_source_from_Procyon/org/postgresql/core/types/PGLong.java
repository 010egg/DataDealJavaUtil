...
public class PGLong implements PGType
{
    ...
    protected PGLong(final Long x) {
        this.val = x;
    }
    
    public static PGType castToServerType(final Long val, final int targetType) throws PSQLException {
        try {
            switch (targetType) {
                case -7: {
                    return new PGBoolean((val == 0L) ? Boolean.FALSE : Boolean.TRUE);
                }
                case 7: {
