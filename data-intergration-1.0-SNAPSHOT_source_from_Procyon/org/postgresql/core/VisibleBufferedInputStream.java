...
public class VisibleBufferedInputStream extends InputStream
{
    ...
    private boolean readMore(final int wanted) throws IOException {
        ...
        final int read = this.wrapped.read(this.buffer, this.endIndex, canFit);
        if (read < 0) {
            return false;
        }
        this.endIndex += read;
    ...
    public int read(final byte[] to, int off, int len) throws IOException {
        ...
        this.endIndex = 0;
        do {
            final int r = this.wrapped.read(to, off, len);
            if (r <= 0) {
