...
public class StreamWrapper
{
    ...
    public StreamWrapper(final InputStream stream) throws PSQLException {
        try {
            ...
            final int memoryLength = this.copyStream(stream, memoryOutputStream, 51200);
            ...
            if (memoryLength == -1) {
                ...
                int diskLength;
                try {
                    diskLength = this.copyStream(stream, diskOutputStream, Integer.MAX_VALUE - rawData.length);
                    if (diskLength == -1) {
                ...
                this.stream = new FileInputStream(tempFile) {
                    ...
                    private void checkShouldClose(final int readResult) throws IOException {
                        if (readResult == -1) {
                            ...
                            if (this._position >= StreamWrapper.this.length) {
                                this.close();
                            }
                        }
                    }
            ...
            throw new PSQLException(GT.tr("An I/O error occurred while sending to the backend."), PSQLState.IO_ERROR, e);
        }
    }
    ...
    private int copyStream(final InputStream inputStream, final OutputStream outputStream, final int limit) throws IOException {
        int totalLength = 0;
