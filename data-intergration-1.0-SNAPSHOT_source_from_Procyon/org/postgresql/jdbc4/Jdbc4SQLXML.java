...
public class Jdbc4SQLXML implements SQLXML
{
    ...
    public Jdbc4SQLXML(final BaseConnection conn) {
        this(conn, null, false);
    }
    
    public Jdbc4SQLXML(final BaseConnection conn, final String data) {
        this(conn, data, true);
    }
    
    private Jdbc4SQLXML(final BaseConnection conn, final String data, final boolean initialized) {
        this._conn = conn;
        this._data = data;
    ...
    @Override
    public synchronized Source getSource(final Class sourceClass) throws SQLException {
        this.checkFreed();
        this.ensureInitialized();
        if (this._data == null) {
            return null;
    ...
    @Override
    public synchronized Result setResult(final Class resultClass) throws SQLException {
        this.checkFreed();
        this.initialize();
