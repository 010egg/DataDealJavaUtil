...
public class JSONStreamReaderUTF16<T> extends JSONStreamReader<T>
{
    ...
    JSONStreamReaderUTF16(final Reader input, final ObjectReaderAdapter objectReader) {
        super(objectReader);
        this.input = input;
        this.context = JSONFactory.createReadContext();
    }
    
    JSONStreamReaderUTF16(final Reader input, final Type[] types) {
        super(types);
        this.input = input;
        this.context = JSONFactory.createReadContext();
    }
    ...
    protected boolean seekLine() throws IOException {
        ...
        while (k < 3) {
            ...
            for (int i = this.off; i < this.end; ++i) {
                if (i + 4 < this.end) {
                    final char b0 = this.buf[i];
                    final char b2 = this.buf[i + 1];
                    final char b3 = this.buf[i + 2];
                    final char b4 = this.buf[i + 3];
                    if (b0 > '\"' && b2 > '\"' && b3 > '\"' && b4 > '\"') {
                        this.lineSize += 4;
                        i += 3;
                        continue;
    ...
    public <T> T readLineObject() {
        ...
        final JSONReader reader = JSONReader.of(this.buf, this.lineStart, this.lineEnd - this.lineStart, this.context);
        Object object;
