...
public final class TLongIntHashMap
{
    ...
    static int nextPrime(final int desiredCapacity) {
        if (desiredCapacity >= TLongIntHashMap.largestPrime) {
            return TLongIntHashMap.largestPrime;
        }
        int i = Arrays.binarySearch(TLongIntHashMap.primeCapacities, desiredCapacity);
        if (i < 0) {
            i = -i - 1;
        }
        return TLongIntHashMap.primeCapacities[i];
    }
    ...
    public TLongIntHashMap(final long key, final int value) {
        this.maxSize = 18;
        this.set = new long[37];
        this.values = new int[37];
        this.consumeFreeSlot = true;
        final int index = ((int)(key ^ key >>> 32) & Integer.MAX_VALUE) % this.set.length;
        this.set[index] = key;
    ...
    public void put(final long key, final int value) {
        final int hash = (int)(key ^ key >>> 32) & Integer.MAX_VALUE;
        ...
        if (isNewMapping) {
            ...
            if (++this.size > this.maxSize || this.free == 0) {
                ...
                final int newCapacity = (this.size > this.maxSize) ? nextPrime(capacity << 1) : capacity;
                final int oldCapacity = this.set.length;
                final long[] oldKeys = this.set;
                final int[] oldVals = this.values;
                ...
                capacity = this.set.length;
                this.maxSize = Math.min(capacity - 1, (int)(capacity * 0.5f));
    ...
    public int get(final long key) {
        ...
        final int setLength = this.set.length;
        final int probe = 1 + hash % (setLength - 2);
        final int loopIndex = index;
        do {
            index -= probe;
            if (index < 0) {
    ...
    public boolean forEachEntry(final BiFunction<Long, Integer, Boolean> procedure) {
        final long[] keys = this.set;
    ...
    public String toString() {
        ...
        this.forEachEntry(new BiFunction<Long, Integer, Boolean>() {
            ...
            @Override
            public Boolean apply(final Long key, final Integer value) {
                if (this.first) {
                    this.first = false;
                }
                else {
    ...
    private int insertKey(final long key) {
        final int hash = (int)(key ^ key >>> 32) & Integer.MAX_VALUE;
        int index = hash % this.set.length;
