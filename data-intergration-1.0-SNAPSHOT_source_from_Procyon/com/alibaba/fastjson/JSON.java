...
public abstract class JSON implements JSONStreamAware, JSONAware
{
    ...
    public static JSONArray parseArray(final String text, final ParserConfig parserConfig) {
        if (text == null) {
            return null;
        }
        final DefaultJSONParser parser = new DefaultJSONParser(text, parserConfig);
    ...
    public static <T> List<T> parseArray(final String text, final Class<T> clazz, final ParserConfig config) {
        if (text == null) {
            return null;
        }
        final DefaultJSONParser parser = new DefaultJSONParser(text, config);
        final JSONLexer lexer = parser.lexer;
    ...
    public static List<Object> parseArray(final String text, final Type[] types, final ParserConfig config) {
        if (text == null) {
            return null;
        }
        final DefaultJSONParser parser = new DefaultJSONParser(text, config);
    ...
    public static byte[] toJSONBytesWithFastJsonConfig(final Charset charset, final Object object, final SerializeConfig config, final SerializeFilter[] filters, final String dateFormat, final int defaultFeatures, final SerializerFeature... features) {
        final SerializeWriter out = new SerializeWriter(null, defaultFeatures, features);
        try {
            final JSONSerializer serializer = new JSONSerializer(out, config);
            if (dateFormat != null && dateFormat.length() != 0) {
    ...
    public static final int writeJSONStringWithFastJsonConfig(final OutputStream os, final Charset charset, final Object object, final SerializeConfig config, final SerializeFilter[] filters, final String dateFormat, final int defaultFeatures, final SerializerFeature... features) throws IOException {
        final SerializeWriter writer = new SerializeWriter(null, defaultFeatures, features);
        try {
            final JSONSerializer serializer = new JSONSerializer(writer, config);
            if (dateFormat != null && dateFormat.length() != 0) {
    ...
    public static Object toJSON(final Object javaObject, final SerializeConfig config) {
        ...
        final String text = toJSONString(javaObject, config, new SerializerFeature[0]);
        return parse(text);
    }
