...
public class DateCodec extends AbstractDateDeserializer implements ObjectSerializer, ObjectDeserializer
{
    ...
    public <T> T cast(final DefaultJSONParser parser, final Type clazz, final Object fieldName, final Object val) {
        ...
        final boolean formatMatch = strVal.length() == dateFomartPattern.length() || (strVal.length() == 22 && dateFomartPattern.equals("yyyyMMddHHmmssSSSZ")) || (strVal.indexOf(84) != -1 && dateFomartPattern.contains("'T'") && strVal.length() + 2 == dateFomartPattern.length());
        if (formatMatch) {
