...
public class ParserConfig
{
    ...
    public ObjectDeserializer getDeserializer(final Class<?> clazz, Type type) {
        ...
        if (deserializer == null && type instanceof ParameterizedTypeImpl) {
            final Type innerType = TypeReference.intern((ParameterizedTypeImpl)type);
        ...
        if (clazz.isEnum()) {
            ...
            final Class mixInType = (Class)JSON.getMixInAnnotations(clazz);
            Class<?> deserClass = null;
            final JSONType jsonType = TypeUtils.getAnnotation((mixInType != null) ? mixInType : clazz, JSONType.class);
            if (jsonType != null) {
            ...
            if (mixInType != null) {
                final Method mixedCreator = getEnumCreator(mixInType, clazz);
    ...
    private static Method getEnumCreator(final Class clazz, final Class enumClass) {
        final Method[] methods = clazz.getMethods();
        Method jsonCreatorMethod = null;
        for (final Method method : methods) {
            if (Modifier.isStatic(method.getModifiers()) && method.getReturnType() == enumClass && method.getParameterTypes().length == 1) {
                final JSONCreator jsonCreator = method.getAnnotation(JSONCreator.class);
                if (jsonCreator != null) {
    ...
    protected ObjectDeserializer getEnumDeserializer(final Class<?> clazz) {
    ...
    public Class<?> checkAutoType(final String typeName, final Class<?> expectClass, final int features) {
        ...
        if (expectClass == null) {
            ...
            final long expectHash = TypeUtils.fnv1a_64(expectClass.getName());
