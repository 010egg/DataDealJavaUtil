...
public class JavaBeanInfo
{
    ...
    public static JavaBeanInfo build(final Class<?> clazz, final Type type, PropertyNamingStrategy propertyNamingStrategy, boolean fieldBased, final boolean compatibleWithJavaBean, final boolean jacksonCompatible) {
        ...
        if ((defaultConstructor == null && builderClass == null) || isInterfaceOrAbstract) {
            final Type mixInType = JSON.getMixInAnnotations(clazz);
            if (mixInType instanceof Class) {
                final Constructor<?>[] mixInConstructors = (Constructor<?>[])((Class)mixInType).getConstructors();
                final Constructor<?> mixInCreator = getCreatorConstructor(mixInConstructors);
            ...
            if (creatorConstructor != null && !isInterfaceOrAbstract) {
                ...
                if (kotlin && constructors.length > 0) {
                    ...
                    creatorConstructor = (Constructor<?>)TypeUtils.getKotlinConstructor(constructors, paramNames);
                    TypeUtils.setAccessible(creatorConstructor);
                }
                else {
                                            ...
                                            if (annotation2 != null || methodName2.startsWith("set")) {
                                                if (builderClass == null) {
                                                    ...
                                                    if (kotlin) {
                                                        ...
                                                        for (int k = 0; k < methods.length; ++k) {
                                                            if (methods[k].getName().startsWith("get")) {
                                                                getMethodNameList.add(methods[k].getName());
                                                            }
                                                        }
                                                    }
                                                    ...
                                                    if (Character.isUpperCase(c2) || c2 > '\u0200') {
                                                        if (kotlin) {
                                                            final String getMethodName = "g" + methodName2.substring(1);
                                                            propertyName4 = TypeUtils.getPropertyNameByMethodName(getMethodName);
                                                        }
                                                        else if (TypeUtils.compatibleWithJavaBean) {
                                                            propertyName4 = TypeUtils.decapitalize(methodName2.substring(3));
                                                        }
                                                        else {
                                                            propertyName4 = TypeUtils.getPropertyNameByMethodName(methodName2);
                                                        }
                                                    }
                                                    else if (c2 == '_') {
                                                        if (kotlin) {
                                                            ...
                                                            if (getMethodNameList.contains(getMethodName)) {
                                                                ...
                                                                propertyName4 = "is" + methodName2.substring(3);
                                                            }
                                                            field2 = TypeUtils.getField(clazz, propertyName4, declaredFields);
                                                        }
                                                        else {
                                                    ...
                                                    if (field2 == null && types4[0] == Boolean.TYPE) {
                                                        final String isFieldName = "is" + Character.toUpperCase(propertyName4.charAt(0)) + propertyName4.substring(1);
                                                        field2 = TypeUtils.getField(clazz, isFieldName, declaredFields);
                                                    }
                                                    JSONField fieldAnnotation3 = null;
                                                    if (field2 != null) {
                                                        fieldAnnotation3 = TypeUtils.getAnnotation(field2, JSONField.class);
                                                        if (fieldAnnotation3 != null) {
                                                            ...
                                                            if (fieldAnnotation3.name().length() != 0) {
                                                                ...
                                                                add(fieldList, new FieldInfo(propertyName4, method2, field2, clazz, type, ordinal5, serialzeFeatures5, parserFeatures5, annotation2, fieldAnnotation3, null, genericInfo));
                                                                break Label_3285;
                                                            }
                                                        }
                                                    }
                                ...
                                if (Collection.class.isAssignableFrom(method3.getReturnType()) || Map.class.isAssignableFrom(method3.getReturnType()) || AtomicBoolean.class == method3.getReturnType() || AtomicInteger.class == method3.getReturnType() || AtomicLong.class == method3.getReturnType()) {
                                    ...
                                    if (annotation3 == null || !annotation3.deserialize()) {
                                        ...
                                        if (annotation3 != null && annotation3.name().length() > 0) {
                                            ...
                                            propertyName5 = TypeUtils.getPropertyNameByMethodName(methodName3);
                                            final Field field4 = TypeUtils.getField(clazz, propertyName5, declaredFields);
                                            if (field4 != null) {
                                                final JSONField fieldAnnotation4 = TypeUtils.getAnnotation(field4, JSONField.class);
                                        ...
                                        if (fieldInfo3 == null) {
                                            add(fieldList, new FieldInfo(propertyName5, method3, collectionField, clazz, type, 0, 0, 0, annotation3, null, null, genericInfo));
                                        }
                                    }
                                }
                            }
